...
reg_poly <- function(poly_order = 1){
    ...
    if (poly_order == 1) {
        ...
        #g2 <- g1 + geom_abline(intercept = parm[1], slope = parm[2], color = "red", size = 2, alpha=0.8)
    } else if (poly_order == 2) {
        ...
        # g2 <- g1 + stat_smooth(method = "lm", se = FALSE, formula = y ~ poly(x, 2), color = "red", size = 2, alpha = 0.8)
    }
